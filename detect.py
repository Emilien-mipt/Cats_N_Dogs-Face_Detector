...
def detect(save_img=False):
    source, weights, view_img, hide_boxes, save_obj, save_txt, imgsz = (
        opt.source,
        opt.weights,
        opt.view_img,
        opt.hide_box,
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_obj:
                        crop_path = os.path.join(
                            opt.project, "_".join([dir_name, "_cropped"])
                        )
                        ...
                        filename = "_".join([p.name.split(".")[0], label, ".jpg"])
                        filepath = os.path.join(os.path.join(crop_path, filename))
                        print("Saving crop {}".format(filename))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--crop", action="store_true", help="save cropped detected objects separately"
    )
